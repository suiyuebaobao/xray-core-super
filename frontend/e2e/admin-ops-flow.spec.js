  ...
  test('covers the core admin operations path without touching production nodes', async ({ page, request }) => {
    ...
    try {
      ...
      await test.step('node CRUD and deploy multi-IP visibility', async () => {
        ...
        const serverRow = tableRowExactText(page, '.admin-nodes', nodeName)
        await expect(serverRow).toBeVisible()
        await expect(serverRow).toContainText('TCP')
        await expect(serverRow).toContainText('XHTTP')
        await expect(serverRow).toContainText('线路 2')
        await serverRow.getByRole('button', { name: '展开当前行' }).click()
        await expect(tableRowExactText(page, '.admin-nodes .line-table', nodeName)).toContainText('TCP')
        await expect(tableRowExactText(page, '.admin-nodes .line-table', `${nodeName}-XHTTP`)).toContainText('XHTTP')
        ...
        await tableRowExactText(page, '.admin-nodes .line-table', nodeName).getByRole('button', { name: '编辑' }).click()
        ...
        await dialog.getByRole('button', { name: '保存' }).click()
        const updatedServerRow = tableRowExactText(page, '.admin-nodes', nodeName)
        await expect(updatedServerRow).toBeVisible()
        await expect(updatedServerRow).toContainText('TCP')
        await expect(updatedServerRow).toContainText('XHTTP')
      ...
      await test.step('redeem code generation and list', async () => {
        ...
        await selectFormOption(page, dialog, '套餐', planName)
        ...
        await expect(page.getByText(`套餐：${planName}`)).toBeVisible()
