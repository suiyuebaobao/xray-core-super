...
test('admin and user pages render against live API', async ({ page }) => {
  ...
  await expect(page.getByLabel('节点显示名称')).toBeVisible()
  await expect(page.getByText('订阅分组')).toBeVisible()
  await expect(page.getByText('自动测速').first()).toBeVisible()
  ...
  await expect(page.getByText('故障转移')).toHaveCount(0)
  await expect(page.getByRole('button', { name: '保存配置' })).toBeVisible()
  ...
  await expect(firstTokenRow.locator('.link-preview')).not.toContainText('/clash')
  await expect(firstTokenRow).not.toContainText('Base64')
  await expect(firstTokenRow).not.toContainText('URI')
